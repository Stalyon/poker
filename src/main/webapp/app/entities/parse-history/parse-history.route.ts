...
import { ParseHistoryDeletePopupComponent } from './parse-history-delete-dialog.component';
...
export class ParseHistoryResolve implements Resolve<IParseHistory> {
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<IParseHistory> {
    const id = route.params['id'];
    if (id) {
      return this.service.find(id).pipe(
        filter((response: HttpResponse<ParseHistory>) => response.ok),
        map((parseHistory: HttpResponse<ParseHistory>) => parseHistory.body)
      );
    }
...
export const parseHistoryPopupRoute: Routes = [
  {
    path: ':id/delete',
    component: ParseHistoryDeletePopupComponent,
    resolve: {
      parseHistory: ParseHistoryResolve
    },
    data: {
